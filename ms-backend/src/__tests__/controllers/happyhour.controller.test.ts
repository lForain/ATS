  ...
  describe('when Happy Hour Create Post route was called', () => {
    ...
    it('should return status true with the respective message when it is a valid description', async () => {
      ...
      for (const reason of result.data) {
        expect(reason).toContainKeys([ '_id', '__v', 'description', 'local', 'eventDate', 'creationDate', 'usersPresent']);
      }

      expect(result2.status).toBe(true);
      expect(result2.message).toInclude('Reason sucessfully created');
      for (const event of result.data) {
        expect(event).toContainKeys([ '_id', '__v', 'description', 'local', 'eventDate', 'creationDate', 'usersPresent']);
      }
    });
